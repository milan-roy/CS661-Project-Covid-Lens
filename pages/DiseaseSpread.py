...
################## Overview ###################################
@st.fragment
def continets_charts(cases_df, deaths_df):
...
def overview(cases_df, deaths_df):
    ...
    continets_charts(cases_df.groupby('Country').tail(1).reset_index(drop=True),
...
########################### Graphs ###############################
@st.fragment
def time_series(cases_df, deaths_df):
...
def plot_graph(cases_df, deaths_df):
    time_series(cases_df,deaths_df)
